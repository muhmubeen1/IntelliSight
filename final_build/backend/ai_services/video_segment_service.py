...
def predict_segment(video_path: str, max_frames: int = 16) -> Dict[str, Any]:
    try:
        vit_result = predict_video(
            video_path,
            frame_skip=5,
            max_frames=max_frames
        )

        i3d_result = i3d_service.predict_video(
            video_path,
            max_frames=32
        )

        fusion_result = fusion_service.fuse_predictions(
            i3d_result,
            vit_result
        )

        print("[DEBUG] I3D:", i3d_result)
        print("[DEBUG] ViT:", vit_result)
        print("[DEBUG] Fusion:", fusion_result)
...
def predict_video_timeline(
    ...
    print(
        f"[Timeline] Original: {total_frames} frames, "
        f"{fps:.2f} fps, {video_duration:.2f}s"
    )
    ...
    print(
        f"[Timeline] {len(segments)} segments "
        f"({segment_duration}s duration, {overlap}s overlap)"
    )
    ...
    for seg in segments:
        seg_path = os.path.join(
            temp_dir,
            f"segment_{seg['index']:04d}.mp4"
        )

        if not extract_segment(
            video_path,
            seg["start"],
            seg["end"],
            seg_path
        ...
        print(
            f"[Timeline] Segment {seg['index']}: "
            f"{result['label']} ({result['confidence']:.2f}) "
            f"[{seg['start']:.1f}s-{seg['end']:.1f}s] "
            f"source={result.get('fusion_source')}"
        )
    ...
    for t in timeline:
        print(
            f"  -> {t['anomaly_type']}: "
            f"{t['start_time']:.1f}s-{t['end_time']:.1f}s "
...
def merge_anomalies(segment_results: List[Dict]) -> List[Dict[str, Any]]:
    abnormal = [
        ...
        if r["label"] != NORMAL_CLASS
        and r["confidence"] >= ANOMALY_THRESHOLD
    ]
